...
def process_data(df, teacher, subject, course, level):
    ...
    for cat in group_order:
        ...
        if weight is not None:
            df_cleaned[avg_col_name] = (raw_avg * weight).apply(custom_round)
        else:
            df_cleaned[avg_col_name] = raw_avg.apply(custom_round)
        ...
        for col in row.index:
            if col.startswith("Average "):
                ...
                if any(cat.lower() == key.lower() for key in weights):
                    ...
                    valid = True
        return int(custom_round(total)) if valid else None
